...
def import_experiment(exp_dir: Path):
    ...
    if existing_exp:
        ...
        experiment_id = client.create_experiment(exp_data["name"], tags=exp_data.get("tags", {}))
...
def import_all_experiments(input_dir: Path):
    ...
    experiment_dirs = [
        d for d in input_dir.iterdir() if d.is_dir() and not d.name.startswith(".") and (d / "experiment.json").exists()
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Import MLflow baseline experiments for local development")
