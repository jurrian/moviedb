...
st.set_page_config(page_title="Movie/Series Search", page_icon="🎬")

st.title("Movie / Series Semantic Search")

st.write(
    "Type what you want to watch (e.g. "
    "`gritty medieval series where a king fights vikings`)."
)
...
if st.button("Search") and query.strip():
    ...
    if not results:
        ...
        for show in results:
            ...
            if show.imdb_rating:
                meta_bits.append(f"IMDb {show.imdb_rating} ({show.imdb_vote_count or 0} votes)")
            if show.tmdb_rating:
                meta_bits.append(f"TMDb {show.tmdb_rating}")

            if meta_bits:
                st.caption(" · ".join(str(x) for x in meta_bits))
